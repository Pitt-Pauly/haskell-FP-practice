-- -*-haskell-*-
--  This is just the SOE-library under a different name.
-- 
--  SOE implementation based on OpenGL and GLFW. This is derived
--  from soegtk (http://haskell.org/~duncan/soegtk/) by Duncan 
--  Coutts.
--
--  Author : Paul Liu
--
...
--
-- (W. Heijltjes, Sept. 2008)
-- Modifications: corrected export of "Graphic (..)"; exported "vertex3"
--
module Pictures (
  runGraphics,
  Title,
...
colorToRGB :: Color -> GL.Color3 Float
...
text (x,y) str = Graphic $ GL.preservingMatrix $ do
  GL.translate (GL.Vector3 (fromIntegral x) (fromIntegral y + 16) (0::Float))
  GL.scale 1 (-1) (1::Float)
...
normaliseBounds :: Point -> Point -> (Float,Float,Float,Float)
